...
| Tricks | `pi/moves.py`: spin, dance, nod, shake. Buttons on the dashboard, keys 1-4 in the simulator. Only play while running; STOP cancels. **`TEMPO` is the single knob for their pace** - at 1.30 every trick is 30% quicker than it was written. Spin is the exception that proves the rule: it cannot just stop sooner or it would end up facing the wrong way, so TEMPO spins it *harder* (`SPIN_TURN` 60 -> 78) and it finishes in 1.41 s instead of 1.80 s, still turning a verified 360 deg. All four end facing where they started, with zero net drift. **At 12 fps this is past what the rhythms can carry**: the shake goes from an even 2 decisions per flick to 2-2-1, the nod loses its mid-pause, and the dance's doubles land 3:1 against the singles instead of 2:1. Everything comes out even again at 20 fps, so this is another thing the frame rate is holding back, not a timing mistake. Drop to `TEMPO = 1.15` if they look ragged before then. |
